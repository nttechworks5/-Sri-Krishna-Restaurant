...
// Optimized: Fast loading, smooth animations, English only, payment flow fixed
...
// ===== Render Menu - Optimized with Skeleton Loading =====
function renderMenu() {
    let items = menuItems;
...
function updateCartDisplay() {
    ...
    cartSubtotal.textContent = 'Rs.' + totalAmount;
    cartGrandTotal.textContent = 'Rs.' + totalAmount;
}
...
function handleUpiAppClick(appId) {
    ...
    if (/Android|iPhone|iPad/i.test(navigator.userAgent)) {
        ...
        showToast(`Opening ${app.name}... Please complete payment`);
    } else {
        showToast('Please open on mobile or scan QR code');
    }

    // Show QR section and enable submit
...
function updatePaymentStatus(status) {
    ...
    if (status === 'paid') {
        el.innerHTML = '<span class="status-paid">Payment Completed - You can now Submit Order</span>';
    } else if (status === 'pending') {
        el.innerHTML = '<span class="status-pending">Select a UPI App and complete payment</span>';
    } else if (status === 'cash') {
...
function submitOrder() {
    ...
    if (method === 'online' && paymentStatus !== 'paid') {
        showToast('Please select a UPI app and complete payment first.');
        return;
    }
